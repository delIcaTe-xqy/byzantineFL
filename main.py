...
from utils.byzantine_fl import krum, trimmed_mean, fang, bulyan, pefl, eppfl, dummy_contrastive_aggregation
...
if __name__ == '__main__':
    ...
    mark_table = torch.zeros(max(int(args.frac * args.num_clients), 1))
    for iter in trange(args.global_ep):
        ...
        elif args.method == 'bulyan':
            w_glob = bulyan(w_locals, compromised_num, args)
        elif args.method == 'pefl':
            w_glob = pefl(w_locals, args)
        elif args.method == 'eppfl':
            w_glob = eppfl(w_locals, mark_table, args)
        ...
        if args.tsboard:
            ...
            writer.add_scalar(f'testloss/{args.method}_{args.p}_cfrac_{args.c_frac}_alpha_{args.alpha}', test_loss,
                              iter)
